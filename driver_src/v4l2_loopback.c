...
// allocator macro:
#define v4l2l_vzalloc(size)  vzalloc(size)

// module parameter defaults:
#define V4L2L_MAX_TIMEOUT       (100000)    // timeout in milliseconds.
#define V4L2L_MAX_BUFFERS       (16)        // max. buffers that will get mapped.
#define V4L2L_MAX_OPENERS       (8)         // max. paralell workers on the device.(greater or equal: 2)
#define V4L2L_MAX_DEVICES       (3)         // how many video devices are initialized.
#define V4L2L_MIN_WIDTH         (48)        // mininal frame width.
#define V4L2L_MIN_HEIGHT        (32)        // minimal frame height.
#define V4L2L_MAX_WIDTH         (8192)      // maximal frame width.
#define V4L2L_MAX_HEIGHT        (8192)      // maximal frame height.
#define V4L2L_DEFAULT_WIDTH     (640)       // default frame width.
#define V4L2L_DEFAULT_HEIGHT    (480)       // default frame height.
#define V4L2L_DEBUG_LEVEL       (0)         // default level of debugging.
#define V4L2L_VERSION_CODE      (1538)      // the version identifier.

// constant device-control ids:
...
#define dprintk(fmt, args ...)                                                               \
    do { if (debug > 0) {                                                                    \
             printk(KERN_INFO "[v4l2-loopback] [" __stringify(__LINE__) "]: " fmt, ## args); \
         } } while (0)
...
// module_param macro:
#define MODULE_PARAM(_name, _desc, _val) \
    static int _name = _val;             \
    module_param(_name, int, S_IRUSR);   \
...
MODULE_DESCRIPTION("v4l2-loopback - a loopback video device");

MODULE_PARAM(width, "frame width", V4L2L_DEFAULT_WIDTH);
MODULE_PARAM(height, "frame height", V4L2L_DEFAULT_HEIGHT);
MODULE_PARAM(debug, "0:off 1:info 2:warn 3:error", V4L2L_DEBUG_LEVEL);
...
struct v4l2_loopback_device {
    ...
    struct v4l2l_buffer		buffers[V4L2L_MAX_BUFFERS];         /* inner driver buffers */
    int						used_buffers;                       /* number of the actually used buffers */
    int						max_openers;                        /* how many times can this device be opened */

    int						write_position;                     /* number of last written frame + 1 */
    struct list_head		outbufs_list;                       /* buffers in output DQBUF order */
    int						bufpos2index[V4L2L_MAX_BUFFERS];    /* mapping of (read/write_position % used_buffers)
...
/** FIXME: dirty fixup for deprecated original do_gettimeofday(struct timspec *) */
static void do_gettimeofday(struct timeval *tv)
{
    struct timespec64 ts;

    ktime_get_real_ts64(&ts);
    tv->tv_sec = (__kernel_time_t)ts.tv_sec;
    tv->tv_usec = (__kernel_suseconds_t)(ts.tv_nsec / 1000L);
}
...
/* device attribute: /sys/devices/virtual/video4linux/videoX/format */
...
/* device attribute: /sys/devices/virtual/video4linux/videoX/buffers */
...
/* device attribute: /sys/devices/virtual/video4linux/videoX/max_openers */
...
#define V4L2_SYSFS_CREATE(x)                   \
    res = device_create_file(&vdev->dev,       \
                             &dev_attr_ ## x); \
    if (res < 0) break
...
/* global module data */
struct v4l2_loopback_device *devs[V4L2L_MAX_DEVICES];
...
static struct v4l2_loopback_device *v4l2loopback_getdevice_internal(int nr)
{
    if (nr < 0 || nr >= V4L2L_MAX_DEVICES) {
        printk(KERN_ERR "v4l2-loopback: illegal device %d\n", nr); return NULL;
    }
...
{
    strlcpy(cap->driver, "v4l2l-Webcam (v4l2-loopback)", sizeof(cap->driver));
    strlcpy(cap->card, "v4l2l-Webcam (v4l2-loopback)", sizeof(cap->card));
    cap->bus_info[0] = 0;

    cap->version = V4L2L_VERSION_CODE;
    cap->capabilities =
                                  ...
                                  struct v4l2_frmsizeenum *argp)
{
    ...
    if (dev->ready_for_capture) {
        ...
        argp->stepwise.min_width = V4L2L_MIN_WIDTH;
        argp->stepwise.min_height = V4L2L_MIN_HEIGHT;

        argp->stepwise.max_width = V4L2L_MAX_WIDTH;
        argp->stepwise.max_height = V4L2L_MAX_HEIGHT;
 ...
 */
static int vidioc_enum_fmt_cap(struct file			*file,
                               void					*fh,
                              ...
                              struct v4l2_format	*fmt)
{
    ...
    if (0 == dev->ready_for_capture) {
        dprintk("[v4l2-loopback] setting fmt_cap not possible yet\n");
        return -EBUSY;
    }
    ...
    do {
        char buf[5]; buf[4] = 0;
        dprintk("[v4l2-loopback] capFOURCC=%s\n", fourcc2str(dev->pix_format.pixelformat, buf));
    } while (0);
 ...
 */
static int vidioc_enum_fmt_out(struct file			*file,
                               void					*fh,
                               struct v4l2_fmtdesc	*f)
{
    struct v4l2_loopback_device *dev;
 ...
 *
 * this should return the currently valid format
                              ...
                              struct v4l2_format	*fmt)
{
    ...
    if (dev->ready_for_capture) {
        ...
        if (w > V4L2L_MAX_WIDTH) w = V4L2L_MAX_WIDTH;
        if (h > V4L2L_MAX_HEIGHT) h = V4L2L_MAX_HEIGHT;

        dprintk("[v4l2-loopback] trying image %dx%d", w, h);

        if (w < 1) w = V4L2L_DEFAULT_WIDTH;
        if (h < 1) h = V4L2L_DEFAULT_HEIGHT;
                            ...
                            struct v4l2_format	*fmt)
{
    ...
    dprintk("[v4l2-loopback] s_fmt_out(%d) %d...%d", ret, dev->ready_for_capture, dev->pix_format.sizeimage);

    do {
        char buf[5];
        buf[4] = 0;
        dprintk("[v4l2-loopback] outFOURCC=%s\n", fourcc2str(dev->pix_format.pixelformat, buf));
    } while (0);
                         ...
                         struct v4l2_streamparm *parm)
{
    ...
    dev = v4l2loopback_getdevice(file);
    dprintk("[v4l2-loopback] vidioc_s_parm called frate=%d/%d\n",
                         ...
                         struct v4l2_control *c)
{
    ...
    case CID_SUSTAIN_FRAMERATE:
        ...
        break;
    case CID_TIMEOUT:
        if (c->value < 0 || c->value > V4L2L_MAX_TIMEOUT)
            return -EINVAL;
        spin_lock_bh(&dev->lock);
                           ...
                           struct v4l2_buffer	*b)
{
    ...
    b->type = type;
    b->index = index;
    dprintkrw("[v4l2-loopback] buffer type: %d (of %d with size=%ld)",
                       ...
                       struct v4l2_buffer	*buf)
{
    ...
    if (buf->index > V4L2L_MAX_BUFFERS)
        return -EINVAL;
    if (opener->timeout_image_io)
        return 0;
    ...
    switch (buf->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
        dprintkrw("[v4l2-loopback] capture QBUF index: %d\n", index);
        set_queued(b);
        return 0;
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
        dprintkrw("[v4l2-loopback] output QBUF pos: %d index: %d\n", dev->write_position, index);
                        ...
                        struct v4l2_buffer	*buf)
{
    ...
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
        ...
        if (index < 0)
            return index;
        dprintkrw("[v4l2-loopback] capture DQBUF pos: %d index: %d\n", opener->read_position - 1, index);
        if (!(dev->buffers[index].buffer.flags & V4L2_BUF_FLAG_MAPPED)) {
            dprintk("[v4l2-loopback] trying to return not mapped buf\n");
            return -EINVAL;
        }
    ...
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
        ...
        dprintkrw("[v4l2-loopback] output DQBUF index: %d\n", b->buffer.index);
        unset_flags(b);
        *buf = b->buffer;
...
static int vidiocgmbuf(struct file			*file,
                       void					*fh,
                       struct video_mbuf	*p)
{
    struct v4l2_loopback_device *dev;
                   ...
                   struct vm_area_struct	*vma)
{
    ...
    if (size > dev->buffer_size) {
        dprintk("[v4l2-loopback] userspace tries to mmap too much, fail\n");
        return -EINVAL;
    }
    if (opener->timeout_image_io) {
        /* we are going to map the timeout_image_buffer */
        if ((vma->vm_pgoff << PAGE_SHIFT) != dev->buffer_size * V4L2L_MAX_BUFFERS) {
            dprintk("[v4l2-loopback] invalid mmap offset for timeout_image_io mode\n");
            return -EINVAL;
        }
    } else if ((vma->vm_pgoff << PAGE_SHIFT) >
               dev->buffer_size * (dev->buffers_number - 1)) {
        dprintk("[v4l2-loopback] userspace tries to mmap too far, fail\n");
        return -EINVAL;
    }
...
static int v4l2_loopback_open(struct file *file)
{
    ...
    if (opener->timeout_image_io) {
        ...
        if (r < 0) {
            dprintk("[v4l2-loopback] timeout image allocation failed\n");
            return r;
        }
    }
    dprintk("[v4l2-loopback] opened dev:%p with image:%p", dev, dev ? dev->image : NULL);
    // TODO: integrate other useable formats !
    {
        ...
        if (0 != vidioc_s_fmt_out(NULL, NULL, &vid_format))
            printk("[v4l2-loopback] setting default format failed!");
        else
            dev->ready_for_capture = 1;
    }
                                  ...
                                  loff_t		*ppos)
{
    ...
    dprintkrw("[v4l2-loopback] leave v4l2_loopback_read()\n");
    return count;
}
                                   ...
                                   loff_t				*ppos)
{
    ...
    dprintkrw("[v4l2-loopback] v4l2_loopback_write() trying to write %zu bytes\n", count);
    if (count > dev->buffer_size)
        count = dev->buffer_size;
    ...
    wake_up_all(&dev->read_event);
    dprintkrw("[v4l2-loopback] leave v4l2_loopback_write()\n");
    return count;
}
...
{
    MARK();
    dprintk("[v4l2-loopback] freeing image@%p for dev:%p", dev ? (dev->image) : NULL, dev);
    if (dev->image) {
        vfree(dev->image);
        dev->image = NULL;
    }
...
static int allocate_buffers(struct v4l2_loopback_device *dev)
{
    ...
    if (dev->image) {
        dprintk("[v4l2-loopback] allocating buffers again: %ld %ld", dev->buffer_size * dev->buffers_number, dev->imagesize);
    ...
    dprintk("[v4l2-loopback] allocating %ld = %ldx%d", dev->imagesize, dev->buffer_size, dev->buffers_number);
    ...
    if (dev->image == NULL)
        return -ENOMEM;
    dprintk("[v4l2-loopback] vmallocated %ld bytes\n",
            dev->imagesize);
    MARK();
    init_buffers(dev);
    return 0;
...
//    b->input             = 0;
    ...
    dev->timeout_image_buffer.buffer.m.offset = V4L2L_MAX_BUFFERS * buffer_size;
    MARK();
}
...
{
    MARK();
    strlcpy(vdev->name, "V4FL-Loopback video device", sizeof(vdev->name));
...
static void init_capture_param(struct v4l2_captureparm *capture_param)
{
    ...
    capture_param->readbuffers = V4L2L_MAX_BUFFERS;
...
static void sustain_timer_clb(unsigned long nr)
{
    ...
    if (dev->sustain_framerate) {
        dev->reread_count++;
        dprintkrw("[v4l2-loopback] reread: %d %d", dev->write_position, dev->reread_count);
                              ...
                              int							nr)
{
    ...
    {
        int ret;
        snprintf(dev->v4l2_dev.name, sizeof(dev->v4l2_dev.name), "v4l2l-Webcam (v4l2-loopback-%03d)", nr);
    ...
    dev->buffers_number = V4L2L_MAX_BUFFERS;
    dev->used_buffers = V4L2L_MAX_BUFFERS;
    dev->max_openers = V4L2L_MAX_OPENERS;
    dev->write_position = 0;
...
int __init init_module(void)
{
    ...
    for (i = 0; i < V4L2L_MAX_DEVICES; i++) {
        dprintk("[v4l2-loopback] creating device#%d\n", i);
        ...
        if (video_register_device(devs[i]->vdev, VFL_TYPE_GRABBER, -1) < 0) {
            ...
            printk(KERN_ERR "[v4l2-loopback] failed video_register_device()\n");
            free_devices();
            return -EFAULT;
        }
    ...
    dprintk("[v4l2-loopback] module installed\n");

    printk(KERN_INFO "[v4l2-loopback] driver version %d.%d.%d loaded\n",
           (V4L2L_VERSION_CODE >> 16) & 0xff,
           (V4L2L_VERSION_CODE >> 8) & 0xff,
           (V4L2L_VERSION_CODE) & 0xff);
...
void __exit cleanup_module(void)
{
    ...
    free_devices();
    dprintk("[v4l2-loopback] module removed\n");
}
